<?php
  ...
  * */
  public function DBInsert($table_name, $item){
    ...
    if ($item != null){
      foreach ($item as $column => $value){
        $statement->bindValue($bind_param_number, $value, PDO::PARAM_STR); // vzdy musim dat value, abych si nesparoval promennou (to nechci)
        $bind_param_number ++;
      }
    }
    // 4) provest dotaz
  ...
  */
  public function DBUpdate($table_name, $set_array, $where_array){
    // PDO - MySQL
    //printr($where_array);
    // vznik chyby v PDO
    $mysql_pdo_error = false;
    ...
    $set_pom = "";
    if ($set_array != null)
      foreach ($set_array as $index => $item){
          if ($set_pom != "") $set_pom .= ", ";
          $column = $item["column"];
          if (key_exists("value", $item))
            $value_pom = "?"; // budu to navazovat
    ...
    // 3) NAVAZAT HODNOTY k otaznikum dle poradi od 1
    $bind_param_number = 1;
    if ($set_array != null || $where_array != null){
      foreach ($set_array as $index => $item){
        if (key_exists("value", $item)){
          $value = $item["value"];
          $statement->bindValue($bind_param_number, $value); // vzdy musim dat value, abych si nesparoval promennou (to nechci)
   ...
   * Smazani radku
   */     
  public function DBDeleteOne($table_name, $where_array){
    // PDO - MySQL
    //printr($where_array);
    // vznik chyby v PDO
    $mysql_pdo_error = false;
  ...
  // ***********************************************************
    /**
    * Pøipojí k vybrané db.
    */
    function Connect(){
      try{
        $options = array(PDO::MYSQL_ATTR_INIT_COMMAND => 'SET NAMES cp1250',);
